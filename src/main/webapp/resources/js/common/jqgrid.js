...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		p["paragonColNames"] = pin.colNames;
		var ts= this, grid={
			headers:[],
			cols:[],
			footers: [],
		...
		var rowHight = ts.p.rowHight;
		var eg = $("<div "+ getstyle(stylemodule, 'entrieBox', false, 'ui-jqgrid row-height-'+rowHight) +"></div>");
		...
		formatter = function (rowId, cellval , colpos, rwdat, _act){
			...
			if(cm !== undefined && cm.formatter !== undefined) {
...
//			console.log(ts.p.colNames);
			while (i<len) {
				...
				for (j=0;j<rowReader.length;j++) {
					v = $.jgrid.getAccessor(cur,rowReader[j]);
					if(ts.p.colModel[j+gi+si+ni] != undefined ){
		...
		populate = function (npage) {
			if(!ts.grid.hDiv.loading) {
				...
				if(pN.sort !== null) {prm[pN.sort]= ts.p.sortname;} if(pN.order !== null) {prm[pN.order]= ts.p.sortorder;}
				 if(pN.pageable !== null) {prm[pN.pageable]= ts.p.pageable;}
				 if(pN.countable !== null) {prm[pN.countable]= ts.p.countable;}
				 if(pN.sortable !== null) {prm[pN.sortable]= ts.p.sortable;}
				...
				case "script":
					$.ajax($.extend({
						...
						success:function(data,st, xhr) {
							ts.p.ori_data = data[ts.p.jsonReader.root];
							...
							if(ts.p.rowClickFocus){
								$(ts).find("tbody > tr.jqgrow").click(function(){
									$(ts).find("tbody > tr.jqgrow").removeClass("focus-row");
...
//			alert(ts.p.colNames);
		...
		ts.p.xmlReader = $.extend(true,{
			...
			id: "[id]",
			userdata: "userdata",
			subgrid: {root:"dt_grid", row: "row", repeatitems: true, cell:"cell"}
		}, ts.p.xmlReader);
		ts.p.jsonReader = $.extend(true,{
			...
			id: "id",
			userdata: "userdata",
			subgrid: {root:"dt_grid", repeatitems: true, cell:"cell"}
		},ts.p.jsonReader);
...
//		});
		...
		var ri,ci, tdHtml;
		var hDivTouchTime = -1; // 2018.10.01
		...
		.dblclick(function(e) {
			...
			/**
			 *  2017.11.23 최범석
			 *  rowEditable일 경우 ondblClickRow이벤트를 선언하여 사용하는 경우
			 *  더블클릭 이벤트 함수가 재 선언 되어서 rowEditable이 정상적으로 작동하지 않아
			 *  rowEditable의 경우 더블클릭 이벤트를 구분하여 사용할수 있도록 ondblClickCustom 선언
			 */
			if($.isFunction(ts.p.ondblClickCustom)){
			    dbcr = ts.p.ondblClickCustom.cal(ts,$(ptr).attr("id"),ri,ci, e);
			    if( dbcr != null) { return dbcr; }
			}
		...
		if(userDevice.search('android') == -1){
			//PDA사용시 2번 처리되는 버그 발생하여 주석
			$(ts).before(grid.hDiv).bind("touchstart", function(e) {
				var $this = $(this), now = new Date().getTime(),
				lastTouchTime = hDivTouchTime,
				...
				if(lastTouchTime != -1){
					...
					if (timeInterval < 500 && timeInterval > 0) {
						...
						if ($tr.length > 0) {
							 ...
							 */
							if($.isFunction(ts.p.ondblClickCustom)){
								dbcr = ts.p.ondblClickCustom.call(ts,$(ptr).attr("id"),ri,ci, e);
								if( dbcr != null) { return dbcr; }
							}
...
//		if(ts.p.domainId){
//			$(grid.cDiv).append(arf).append("<span class='ui-jqgrid-title' data-domain-id='"+ts.p.domainId+"' ></span>").addClass("ui-jqgrid-titlebar ui-jqgrid-caption"+(dir==="rtl" ? "-rtl" :"" )+" "+getstyle(stylemodule,'gridtitleBox',true));
//		}else{
			$(grid.cDiv).append(arf).append("<span class='ui-jqgrid-title' id='"+ts.p.id+"_caption' >"+ts.p.caption+"</span>").addClass("ui-jqgrid-titlebar ui-jqgrid-caption"+(dir==="rtl" ? "-rtl" :"" )+" "+getstyle(stylemodule,'gridtitleBox',true));
//		}
		var reportBtnGroup = $('<div class="report-btn-group '+ (ts.p.hidegrid===true? '':'no-m' )+' "  >');
		if(ts.p.reportPdfBtn){
			var reportPdfBtn = $('<i class="fa fa-file-pdf-o" style=""></i>');
			reportPdfBtn.click(function(){
				if(ts.p.onClickPdfBtn){
					ts.p.onClickPdfBtn(p);
				}else{
					ts.p.onClickPdfBtnDefault(p);
				}
//				alert(ts.p.caption+" : Pdf");
			});
			reportBtnGroup.append(reportPdfBtn);
		}
		if(ts.p.reportExcelBtn){
			var reportExcelBtn = $('<i class="fa fa-file-excel-o" style=""></i>');
			reportExcelBtn.click(function(){
				if(ts.p.onClickExcelBtn){
					ts.p.onClickExcelBtn(p);
				}else{
					ts.p.onClickExcelBtnDefault(p);
				}
//				alert(ts.p.caption+" : Excel");
			});
			reportBtnGroup.append(reportExcelBtn);
		}
		$(grid.cDiv).append(reportBtnGroup);
		...
		$(document).bind( "mouseup.jqGrid" + ts.p.id, function (e) {
					...
					if(t.rows.length === 0){
						...
						switch (pos) {
							...
							case 'after':
								...
								console.log('after sind', sind, src)
								if (sind) {
...
//		resortArray(ts.p.colNames);
		resortArray(ts.p.paragonColNames);
...
//				thStyle = { height: '0px', width: ths[i].width*0.99+ 'px', display: (cmi.hidden ? 'none' : '')};
				...
				if (iCol >= 0) {
					...
					$colHeader = $('<th>').attr({role: "columnheader"})
						...
						.html(titleText);
//					$colHeader.css({"max-width":(cmi.fixed ? cmi.width: '')});
...
$.jgrid.extend({
	pivotSetup : function( data, options ){
		...
		return { "colModel" : columns, "dt_grid": pivotrows, "groupOptions" : groupOptions, "groupHeaders" :  headers, summary : summaries };
	},
