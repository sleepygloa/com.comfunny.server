...
var handleSidebarMenu = function() {
    "use strict";
    $('.sidebar .nav > .has-sub > a').off().click(function() {
    ...
    $('.sidebar .nav > .has-sub .sub-menu li.has-sub > a').off().click(function() {
        if ($('.page-sidebar-minified').length === 0) {
            var target = $(this).next('.sub-menu');
...
var handleMobileSidebarToggle = function() {
    ...
    $('.sidebar').on('click touchstart', function(e) {
        if ($(e.target).closest('.sidebar').length !== 0) {
            ...
            e.stopPropagation(); // 2018. 10. 02 추가
        } else {
...
//json Parameter 사용
var defaultAjaxSetting = function() {
	$.ajaxSetup({
//		global: false,
		timeout: 6000000,
		type : "POST",
...
//		contentType: 'application/json; charset=utf-8',
		...
		beforeSend : function(xhr){
			...
			var proNm = $(".main-tab.active > a").data("proNm");
			if(proCd == undefined){
				proCd = 'PC0001';
				proNm = 'Dashboard';
			}

//			var proCd = $("a[class=active]").data("procd"); /* Tab없는 메뉴 */
//			var proNm = $("a[class=active]").data("proNm"); /* Tab없는 메뉴 */
			xhr.setRequestHeader("AjaxType", "paragon");
			xhr.setRequestHeader("proCd", proCd);
			xhr.setRequestHeader("proNm", proNm);
//			App.prcsStart();
    	},error : function(jqXHR, textStatus, errorThrown) {
...
var App = function () {
	...
	return {
		...
		getCallBackEl: function () {
			return $callBackEl01;
		},
		setCallBackEl02: function (el) {
		    $callBackEl02 = el;
		},
		getCallBackEl02: function () {
		    return $callBackEl02;
		},
		setTargetGrid: function (grid) {
			$callBackGrid = grid;
		},
		setElId: function (key,elId) {
			callBackEls[key]=elId;
		},
		setElIds: function (obj,elId) {
			callBackEls = obj;
		},
		getElId: function (elId) {
			var callBackEl = callBackEls[elId];
			if(callBackEl){
				return callBackEl;
			}else{
				alert("객체가 없습니다.");
		...
		callBackCasting: function (data) {
			if(callBackEls){
				$.each( callBackEls, function( key, targetKey ) {
					var targetEl ;
					if($callBackGrid){
						targetEl = $callBackGrid.find("#"+targetKey);
						if(!targetEl){
							targetEl = $("#"+targetKey);
						}
					}else{
					...
					if(targetEl.is("input")){
						targetEl.val(data[key]);
					}else if($callBackGrid){
						var taRowid = (targetKey.substring(0,targetKey.indexOf("_")));
						var taColNm = (targetKey.substring(targetKey.indexOf("_")+1));
						$callBackGrid.jqGrid('setCell',taRowid,taColNm,data[key]);
					}
				});
				callBackEls = null;
			}
...
//            });
		...
		restartDraggablePanel: function(tabId) {
//			handleDraggablePanel(tabId);
		},
		prcsStart: function() {
			$('#ajax-loader').removeClass('hide');
			setTimeout(function() {
				if(!$('#ajax-loader').hasClass("hide")){
					$('#ajax-loader').addClass('hide');
				}
            }, 6000000);
		},
		prcsEnd: function(tabId) {
			if(!$('#ajax-loader').hasClass("hide")){
				$('#ajax-loader').addClass('hide');
			}
		},
		restarLocalStorage: function(tabId) {
			handleLocalStorage(tabId);
		}
    };
}();
